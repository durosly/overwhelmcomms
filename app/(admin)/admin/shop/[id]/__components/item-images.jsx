...
function ItemImages({ id }) {
	...
	const { isPending: isPendingMutation, mutate } = useMutation({
		mutationFn: (image) => {
			toastId.current = toast.loading("Deleting item image...");
			return axios.delete(`/api/admin/item/${id}/images`, {
				data: { images: [image] },
			});
		},
		...
		onSuccess: () => {
			toast.success("Item image deleted", { id: toastId.current });
		},
		onError: (error) => {
...
function UploadFile({ file, id }) {
	...
	async function saveToDb(public_id) {
		...
		setSaving(true);
		try {
			const response = await axios.post(`/api/admin/item/${id}/images`, {
				images: [public_id],
			});
